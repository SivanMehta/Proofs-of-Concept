...
for combo in species:
    dex.append(combo.split(" "))

class pokeType():
    def __init__(self, name, weaknesses, resistances, immunities):
        self.name = name
...
normal = pokeType("Normal", ["Fighting"], [], ["Ghost"])
grass = pokeType("Grass", ["Fire", "Bug", "Flying", "Poison", "Ice"], ["Grass","Water","Electric", "Ground"], [])
fire = pokeType("Fire", ["Water", "Ground", "Rock"], ["Fire", "Grass", "Steel", "Ice", "Fairy", "Bug"], [])
water = pokeType("Water", ["Grass", "Electric"], ["Water", "Fire", "Steel", "Ice"], [])
fighting = pokeType("Fighting",["Psychic", "Flying", "Fairy"],["Rock", "Bug", "Dark"],[])
flying = pokeType("Flying",["Electric", "Rock", "Ice"],["Fighting", "Grass", "Bug"],["Ground"])
poison = pokeType("Poison", ["Psychic", "Ground"], ["Poison", "Fighting", "Fairy", "Bug", "Grass"], [])
electric = pokeType("Electric",["Ground"],["Electric", "Steel", "Flying"], [])
ground = pokeType("Ground", ["Water", "Grass", "Ice"], ["Poison", "Rock"], ["Electric"])
psychic = pokeType("Psychic", ["Dark", "Ghost", "Bug"], ["Psychic", "Fighting"], [])
rock = pokeType("Rock",["Water", "Grass", "Fighting", "Ground"],["Fire", "Normal", "Flying", "Poison"],[])
ice = pokeType("Ice", ["Fire", "Fighting", "Steel", "Rock"], ["Ice"], [])
bug = pokeType("Bug", ["Fire", "Rock", "Flying"], ["Fighting", "Ground", "Grass"], [])
dragon = pokeType("Dragon", ["Dragon", "Ice", "Fairy"], ["Fire", "Water", "Grass", "Electric"], [])
ghost = pokeType("Ghost", ["Ghost", "Dark"], ["Bug", "Poison"], ["Fighting", "Normal"])
dark = pokeType("Dark", ["Fighting", "Bug", "Fairy"], ["Dark", "Ghost"], ["Psychic"])
fairy = pokeType("Fairy", ["Steel", "Poison"], ["Bug", "Dark", "Fighting"], ["Dragon"])
steel = pokeType("Steel", ["Fighting", "Fire", "Ground"], ["Normal", "Grass", "Flying", "Psychic", "Rock", "Ice", "Bug", "Dragon", "Fairy", "Steel"], ["Poison"])

pokeTypes = [normal, grass, fire, water, fighting, flying, poison, electric, 
         ground, psychic, rock, ice, bug, dragon, ghost, dark, fairy, steel]

import copy
def generateRelationships(type1, type2):
    weaknesses = []
    resistances = []
    immunities = []
    # add everything from 1
    weaknesses = copy.deepcopy(type1.weaknesses)
    resistances = copy.deepcopy(type1.resistances)
    immunities = copy.deepcopy(type1.immunities)
    ...
    for immunity in type2.immunities:
        if(immunity in weaknesses):
            weaknesses.remove(immunity)
            immunities.append(immunity)
        elif(immunity in resistances):
            resistances.remove(immunity)
            immunities.append(immunity)
        elif(immunity in immunities):
            pass
        # at this point, it has become a neutrality
        else:
            immunities.append(immunity)

    for resistance in type2.resistances:
        if(resistance in resistances):
            resistances.remove(resistance)
            resistances.append(resistance + " x 2") # double resistance
        elif(resistance in weaknesses):
            weaknesses.remove(resistance)
        elif(resistance in immunities): pass
        else:
            resistances.append(resistance)

    for weakness in type2.weaknesses:
        if(weakness in weaknesses):
            weaknesses.remove(weakness)
            weaknesses.append(weakness + " x 2") # double weakness
        elif(weakness in resistances):
            resistances.remove(weakness)
        elif(weakness in immunities): pass
        else:
...
class Pokemon():
    def __init__(self, name, type1, type2 = None):
        ...
        if(type1 in pokeTypes and type2 in pokeTypes):
            pass
        else:
            raise Exception("Not valid input type")

        self.type1 = type1
        if(type2 == None or type1 == type2):
        ...
        relationships = generateRelationships(self.type1, self.type2)

        self.resistances = relationships[0]
        self.weaknesses = relationships[1]
        self.immunities = relationships[2]
    ...
    def __str__(self):
        ...
        relationships = "Resistances: " + str(self.resistances) + "\n"
        relationships += "Weaknesses: " + str(self.weaknesses) + "\n"
        relationships += "Immunities: " + str(self.immunities)
...
for combo in dex:
    ...
    livingDex.append(Pokemon(combo[0], eval("%s" % combo[1].lower()), eval("%s" % combo[2].lower())))
    # eval("%s" % combo[1].lower()) is to handle the fact that the types above are giving in capitalized strings, but the they need to
    # be interpreted as objects which have a lower case name

#################################################
## livingDex is now a complete dex of pokemon! ##
#################################################
